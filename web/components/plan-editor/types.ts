...
export function weekGroupsToEntries(weekGroups: WeekGroup[]): PlanEntryCreate[] {
...
export function entriesToWeekGroups(entries: PlanEntryCreate[]): WeekGroup[] {
  if (entries.length === 0) {
    return [{ name: 'Week 1', days: [] }];
  }

  const weekMap = new Map<string, Map<string, PlanEntryCreate[]>>();
