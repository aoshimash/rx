...
type ProgramRepository interface {
	...
	// GetByID retrieves a Program by its ID including all ProgramNode records in tree structure, returns domain.ErrNotFound if not found
	GetByID(ctx context.Context, id uuid.UUID) (*domain.Program, error)

	// Update replaces an existing Program including all ProgramNode records, returns domain.ErrNotFound if not found
	Update(ctx context.Context, program *domain.Program) error

	// Delete removes a Program by ID (cascades to ProgramNode records), returns domain.ErrNotFound if not found
